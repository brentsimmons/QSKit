...
} QSTimeZoneAbbreviationAndOffset;


#define kNumberOfTimeZones 96

static const QSTimeZoneAbbreviationAndOffset timeZoneTable[kNumberOfTimeZones] = {
...
static NSInteger nextMonthValue(const char *bytes, NSUInteger numberOfBytes, NSUInteger startingIndex, NSUInteger *finalIndex) {
	...
	return QSJanuary; //should never get here
}
...
static NSInteger offsetInSecondsForTimeZoneAbbreviation(const char *abbreviation) {
	...
	for (i = 0; i < kNumberOfTimeZones; i++) {
		...
		QSTimeZoneAbbreviationAndOffset zone = timeZoneTable[i];
...
static NSDate *QSParsePubDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {
...
static NSDate *QSParseW3CWithBytes(const char *bytes, NSUInteger numberOfBytes) {
...
NSDate *QSDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {
	...
	if (dateIsPubDate(bytes, numberOfBytes))
		return QSParsePubDateWithBytes(bytes, numberOfBytes);

	return QSParseW3CWithBytes(bytes, numberOfBytes);
}
...
NSDate *QSDateWithString(NSString *dateString) {
	...
	return QSDateWithBytes(utf8String, strlen(utf8String));
}
